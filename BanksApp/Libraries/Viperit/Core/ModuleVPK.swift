...
public struct ModuleVPK {
    ...
    static func build<T: RawRepresentable & ViperitModule>(_ module: T, bundle: Bundle = Bundle.main, deviceType: UIUserInterfaceIdiom? = nil) -> ModuleVPK where T.RawValue == String {
        // Get class types
...
extension ModuleVPK {
    static func loadView<T: RawRepresentable & ViperitModule>(forModule module: T, bundle: Bundle, deviceType: UIUserInterfaceIdiom? = nil) -> UserInterfaceProtocol where T.RawValue == String {
        let viewClass = module.classForViperComponent(.viewController, bundle: bundle, deviceType: deviceType) as! UIViewController.Type
    ...
    static func build(view: UserInterfaceProtocol, interactor: InteractorProtocol, presenter: PresenterProtocol, router: RouterProtocol, displayData: DisplayData?) -> ModuleVPK {
        // View connections
        view._presenter = presenter
        view._displayData = displayData
        ...
        return ModuleVPK(view: view, interactor: interactor, presenter: presenter, router: router, displayData: displayData)
    }
}
